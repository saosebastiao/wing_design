...
def eigen_worst_mode(model, loads_aero, r, rho):
...
def main() -> None:
    ...
    # 2. rebuild model + loads + pressures EXACTLY as multistart_v2.py
    P = medium_scenario()
    spec = P.geometry
    ...
    lam, mode = eigen_worst_mode(model, loads, r, P.rho_kgm3)
    ...
    # 4. worst-mode VTU
    vtu_path = out / "best_medium_worst_mode.vtu"
    if mode is None:
    ...
    try:
        ...
        kind = "circular"
    beams_path = out / "best_medium_beams.stl"
    comp = Compound(label="best_medium_beams", children=list(beams))
    ...
    print("\n=== EXPORT SUMMARY (1021.6 kg medium headline) ===", flush=True)
