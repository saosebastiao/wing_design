...
def provider_brace_vm(sigma_allow, brace_r_min, brace_element_indices):
    """KS provider: von-Mises utilisation on lateral-bracing ring elements.

    The brace section radius is FIXED (``brace_r_min``) — it is NOT a design
    variable for this constraint.  Therefore ``expl_list`` is always empty and
    the gradient flows only through the displacement adjoint (same adjoint path
    as the rest of the beam_vm family, but without a radius-group column hit).
    ...
    sigma_allow : float
        Allowable stress [Pa].
    brace_r_min : float
        Fixed nominal radius of brace elements [m].
    brace_element_indices : array-like
        Indices of the brace elements into ``fac.beam_elements``
        (= ``model.brace_elements``).
    """
    brace_els = np.asarray(brace_element_indices, dtype=int)
    r = float(brace_r_min)
    A = np.pi * r ** 2
    Iz = np.pi * r ** 4 / 4.0
    J = np.pi * r ** 4 / 2.0
    ...
    def p(fac, ds):
        ...
        vals = np.empty(K)
        dofs_l, dgdu_l, expl_l = [], [], []
        for k, e in enumerate(brace_els):
            floc, Mmat, dofs = _active_beam_force(fac, int(e))
            axial = floc[6]
            torsion = floc[9]
            b0 = np.hypot(floc[4], floc[5])
            b1 = np.hypot(floc[10], floc[11])
            ...
            tau = abs(torsion) * r / J
            vm = np.sqrt(sigma_n ** 2 + 3.0 * tau ** 2)
            vals[k] = vm / sigma_allow
            if vm == 0.0:
                dofs_l.append(dofs)
                dgdu_l.append(np.zeros(12))
                expl_l.append([])
                continue
            # ∂(vm/σ)/∂floc — same formula as _beam_vm_grad_one in laminate_sizing
            dvm_dfloc = np.zeros(12)
            sgn_ax = np.sign(axial) if axial != 0.0 else 0.0
            sgn_tor = np.sign(torsion) if torsion != 0.0 else 0.0
            dvm_dfloc[6] = (sigma_n / vm) * sgn_ax / A
            ...
            if Mres > 0.0:
                ...
                dvm_dfloc[ib] = coef * (fb / Mres)
            dofs_l.append(dofs)
            dgdu_l.append((dvm_dfloc @ Mmat) / sigma_allow)
            # No explicit DV terms: brace radius is FIXED, not a design variable.
            expl_l.append([])
        return vals, dofs_l, dgdu_l, expl_l
    return p
